...
2.	Navigate to the section ‘Define comparisons and paramters’ and modify the required input accordingly.
    ...
    6.	`organism_annotation`: definition of your organism of interest. Options: "mouse", "human", "medicago_trucantula", "rhizobium_melitoli", "pig", "human_iso", "ecoli" and "arabidopsis".
...
3.	Navigate to the section “Define comparisons and parameters”, and modify accordingly. You can follow point 2 described in the section Exploratory Analysis Script of this documentation until instrument.
    ...
    1.	`targetp_location`: here you can define the relative path to a `.targetp2` file, holding the N-termini annotations for your organism of interest. You can either download a `.targetp2` file from https://services.healthtech.dtu.dk/services/TargetP-2.0/ (if your organism of interest is available) or generate your own `.targetp2` file by running TargetP 2.0 locally to annotate the sequences of your organism of interest (i.e., from a fasta file downloaded from UniProt). TargetP 2.0 execution in Linux is described in the software’s README file shipped with the installation.
    2.	`defined_contrasts`: here you need to define the comparisons you want to make with your limma analysis. In the example below, the left side of the equal sign defines an arbitrary name you give to the contrast, and “Oct1 – WT” on the right side, define what’s need to be compared. These need to match exactly the name of your conditions as defined in your experiment metadata/annotation file.
    ...
    3. `fc_threshold` and `pval_threshold`. Modify according to your experiment. Normally pval threshold be 0.05.
    4. `pre_fix`: here you would define a prefix for the output files generated by the script. Normally it should be meaningful for your experiment.
    5. `keep_only`: here you would list the modifications that you would consider for your final HUNTER analysis. "Dimethyl", "Acetyl" means that you would keep only features that contain either dimethyl or acetyl at the N-termini.
    6. `exclude_preceding`: if your undecanal pullout efficiency is not so great (you have a lot of labelled peptides that potentially come from tryptic digestion; you see that after the exploratory script and the labelling efficiency script), you can exclude peptides that have “R” as a preceding amino acid. Change to NULL (without quotation marks), if you don’t want to exclude features by their preceding amino acid.

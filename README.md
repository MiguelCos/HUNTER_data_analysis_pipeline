...
```
  ...
  1. `fc_threshold` and `pval_threshold`. Modify according to your experiment. Normally pval threshold be 0.05.
  2. `pre_fix`: here you would define a prefix for the output files generated by the script. Normally it should be meaningful for your experiment.
  3. `keep_only`: here you would list the modifications that you would consider for your final HUNTER analysis. "Dimethyl", "Acetyl" means that you would keep only features that contain either dimethyl or acetyl at the N-termini.
  4.   `exclude_preceding`: if your undecanal pullout efficiency is not so great (you have a lot of labelled peptides that potentially come from tryptic digestion; you see that after the exploratory script and the labelling efficiency script), you can exclude peptides that have “R” as a preceding amino acid. Change to NULL (without quotation marks), if you don’t want to exclude features by their preceding amino acid.

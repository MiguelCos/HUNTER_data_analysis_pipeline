...
The exploratory and inferential HUNTER scripts expect peptide-level quantitation from DIA-NN, sample metadata, and a FASTA file matching the searched proteome. Peptide specificity, UniProt processing annotations, MEROPS information, and TargetP2-derived processing annotations are added through `TermineR::annotate_neo_termini()`.
...
You need to have the following installed in your computer (I am listing the versions of all of these programs, which at the time of writing of this documentation are working properly):
...
Use the current GitHub version of `MiguelCos/TermineR`. The HUNTER workflows now rely on `annotate_neo_termini()` to add TargetP2-related columns directly, including `targetp_category`, `targetp_p1_position`, `targetp_matches_p1_prime`, `processing_type_targetp`, `processing_type_terminer`, `processing_type`, and `source_processing_annotation`.

### Required workflow inputs

At minimum, the exploratory and inferential reports require:

- `diann_report_location`: path to the DIA-NN `report.tsv` produced from the FragPipe-DIA search.
- `location_annotation`: path to the sample metadata file in TSV format.
- `fasta_location`: path to the FASTA file used for, or compatible with, the search. The FASTA should use UniProt-style headers so accessions can be matched to TermineR annotations.
- `sense_protease`: `"C"` for proteases cutting on the carboxyl side, `"N"` for proteases cutting on the amino side.
- `specificity_protease`: the cleavage specificity pattern, for example `"K|R"` for trypsin.
- `organism_annotation`: the organism key used by TermineR to select UniProt and TargetP2 annotation resources.
- `instrument`: `"EX"` for Exploris-style sample columns or `"QE"` for Q-Exactive-style sample columns.
- `missing_accepted`: the maximum accepted missingness per condition, expressed as a fraction such as `1 / 4`.

The inferential report also requires:

- `defined_contrasts`: named limma contrasts matching the condition names in the metadata file.
- `fc_threshold` and `pval_threshold`: thresholds used for regulation labels and visualizations.
- `pre_fix`: prefix used for generated output tables and figures.
- `keep_only`: N-terminal modifications retained for inferential analysis, for example `c("Dimethyl", "Acetyl")`.
- `exclude_preceding`: amino acids excluded at the preceding position, or `NULL` to disable this filter.

TargetP2 files no longer need to be supplied separately in the HUNTER notebooks. When TargetP2 processing data are available in the installed TermineR package for the selected `organism_annotation`, `annotate_neo_termini()` adds them automatically. If TargetP2 data are not available for an organism, the workflows continue with UniProt-derived processing annotation.

### Experimental metadata/sample annotation file

You need a sample metadata/annotation file in .tsv format. It should contain AT LEAST the columns ‘sample’ and ‘condition’ (exactly this name). We have an example file in the `example_files` folder: `example_sample_metadata_annotation.tsv`; you can modify it according to your experiment. Here I describe each column and what is expected to have.
...
3. Within that folder copy the qmd files for your data analysis (our HUNTER scripts)
   1. From this repo, you can download the scripts and put them in your project folder.
   2. To download the individual scripts, click on the `.qmd` file link, and then click on the ‘raw’ button. This will open the script in a new tab. You can then right-click and select ‘save as’ to download the script.
   3. You can also download the entire repo as a zip file and extract it in your project folder (see the green ‘Code’ button in the top right corner of this page).
...
2.	Navigate to the section ‘Define interesting variables’. This is where you can the modifications that are more interesting to you.
   1.  Change the input for the ‘analysis_path’ object. If you correctly set up your RStudio project, you can use a relative path to the folder containing your FragPipe search.
   2.  Modify each other input in this chunk according to the expected modification mass of your experiment. Keep the format for the script for the mass and N-terminal and K modification mass.
   3.  Modify the specificity of your experimental protease (`“K|R”` is trypsin).
...
2.	Navigate to the section ‘Define comparisons and paramters’ and modify the required input accordingly.
    1.	`diann_report_location`: can be defined as a relative path to the report.tsv file from your RProj file.
    2.	`location_annotation`: relative path to your sample metadata/annotation file (see above on how to create this file).
    3.	`fasta_location`: relative path to a fasta file containing at least all the protein sequences of proteins identified in your FragPipe search. All FragPipe searches usually include a ‘protein.fas’ file. You can also use the UniProt fasta file for your organism of interest. Note: The pipeline only works with UniProt type of sequences and annotations
    4.	`sense_protease`: “C” if your experimental protease cuts and the carboxylic side (like trypsin), “N” if it cuts at the aminic side.
    5.	`specificity_protease`: same as described for the labelling efficiency script (“K|R” is trypsin).
    6.	`organism_annotation`: definition of your organism of interest. Options currently supported by TermineR include "mouse", "human", "medicago_trucantula", "rhizobium_melitoli", "pig", "human_iso", "arabidopsis", "ecoli", "rat", "yeast", "c_elegans", and "synechocystis".
    7.	`instrument`: should match the initials that are used for the naming of our MS files. “EX” for exploris, “QE” for Q-exactive…
    8.	`missing_accepted`: should be defined as the proportion of missing values accepted by the user. Example: `1 / 4` would mean that a feature can be missing in up to 1 out of 4 replicates per condition. If it is missing in more than 1 replicate, it would be considered as 'completeley missing' in one condition.
    9.	`tune_sigma` and tune_quantile: tune sigma is recommended to leave as is, and tune_quantile should be set as a very low number < `0.0001`. Specially if you are interested in seen differential abundance of features that are already lowly abundant.
...
2.	Navigate to the section “Required R packages”. In the ‘Variable’ subsection, define your organism database R package:
...
3.	Navigate to the section “Define comparisons and parameters”, and modify accordingly. You can follow point 2 described in the section Exploratory Analysis Script of this documentation until instrument. TargetP2 annotation is handled by `annotate_neo_termini()` through the installed TermineR package, so there is no separate `targetp_location` input in the current workflow.


    1.	`defined_contrasts`: here you need to define the comparisons you want to make with your limma analysis. In the example below, the left side of the equal sign defines an arbitrary name you give to the contrast, and “Oct1 – WT” on the right side, define what’s need to be compared. These need to match exactly the name of your conditions as defined in your experiment metadata/annotation file.
    ...
    2. `fc_threshold` and `pval_threshold`. Modify according to your experiment. Normally pval threshold be 0.05.
    3. `pre_fix`: here you would define a prefix for the output files generated by the script. Normally it should be meaningful for your experiment.
    4. `keep_only`: here you would list the modifications that you would consider for your final HUNTER analysis. "Dimethyl", "Acetyl" means that you would keep only features that contain either dimethyl or acetyl at the N-termini.
    5. `exclude_preceding`: if your undecanal pullout efficiency is not so great (you have a lot of labelled peptides that potentially come from tryptic digestion; you see that after the exploratory script and the labelling efficiency script), you can exclude peptides that have “R” as a preceding amino acid. Change to NULL (without quotation marks), if you don’t want to exclude features by their preceding amino acid.
...
4.	You should not need to change anything from the script until the section “Visualization of differential abundance analysis”.
    1.	Here, in the subsections ‘Basic’ and ‘Labelled’ volcano, you should change the section of the code that controls the labels of the plot in the ‘labs’ function.

5.	You might get errors in the section “Functional annotation of 'substrates”.
    1.	The main reason is that within the chunks it evaluates the presence of intermediary files in a specific folder that it expects to find within your RStudio folder. `r_project_files/rds/…` If you create these folder structure as recommended in the usability notes, this should be solved.
...
6.	After the section Heatmaps of cleavage areas of differentially abundant termini, you might want to change the titles of the following subsections, and you will need to change the definition of the `contrast` that you want to highlight in your cleavage area heatmap.
    1.	You might also want to change the title of the heatmaps in the argument `main` within the pheatmap function.
...
8.	In the section Sequence logos of cleavage areas of differentially abundant termini, we also extract sequences of cleavage areas and visualize them using sequence logos.
    1.	For sequence logos, we need to build a background model with sequences of the organism of interest.
    2.	You would need to define the organism in the `species` argument within the function `prepareProteome` in the first chunk.
    3.	Following code should work with the same input of sequences generated for the heatmaps.
